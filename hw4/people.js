  ...
  .post("/people/add", async (ctx) => {
    ...
    if (body.type === "form") {
      ...
      if (peoples.get(name)) {
        ...
        ctx.response.body = `<p>此帳號已經註冊過，註冊失敗</p><p><a href="http://127.0.0.1:8000/public/add.html">請重新註冊一次</a></p>`
      } 
      else {
        peoples.set(name, {name, password})
        ctx.response.type = 'text/html'
        ctx.response.body = `<p>您已註冊成功</p><p><a href="http://127.0.0.1:8000/public/index.html">請登入</a></p>`
      }
  ...
  .post("/people/find", async (ctx) => {
    ...
    if (body.type === "form") {
      ...
      if (peoples.get(name) && password == peoples.get(name).password) {
        ctx.response.type = 'text/html'
        ctx.response.body = `<p>登陸成功</p>`
      } 
      else {
        ctx.response.type = 'text/html'
        ctx.response.body = `<p>密碼錯誤，登陸失敗</p><p><a href="http://127.0.0.1:8000/public/index.html">請重新登入</a></p>`
      }
